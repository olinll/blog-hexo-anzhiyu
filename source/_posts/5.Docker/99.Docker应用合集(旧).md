...
title: Docker应用合集(旧)
aliases:
date: 2025-12-27T16:29
updated: 2025-12-27T16:30
categories:
  - Docker
tags:
  - Docker
...
在家庭数据中心使用Docker，最核心的好处在于它通过容器化技术实现了应用的**隔离、便携和高效管理**。你可以将各种家庭服务（如媒体库、智能家居、下载工具等）封装在独立的容器中，彼此互不干扰，彻底摆脱了依赖冲突和环境配置的噩梦；使用Docker Compose等工具，只需一个配置文件就能一键部署或更新整个复杂应用栈，极大简化了维护工作。此外，Docker容器轻量级、低开销的特性，让你能在性能有限的家庭服务器（甚至是一台树莓派）上轻松运行大量服务，并且能方便地进行备份、迁移和快速恢复，极大地提升了家庭数据中心的可靠性、可管理性和资源利用率，让家庭服务器变得像专业数据中心一样井井有条。

在这里推荐一个镜像站：[渡渡鸟镜像同步站]([https://docker.aityp.com](https://docker.aityp.com))

Docker官方镜像仓库[Docker-Hub](http://hub.docker.com)
...
services:
  dockge:
    image: 'louislam/dockge:1.5.0' # 镜像
    container_name: dockge # 容器名称
    hostname: dockge  # 容器host名称
    restart: always # 启动策略
    ports: # 端口映射 主机端口:容器内部端口
      - 18001:5001
    volumes:  # 映射目录  主机目录:容器内目录
...
## Portainer
Portainer CE是一个轻量级、开源的管理UI，用于简化Docker环境的管理和监控。通过它，您可以轻松管理Docker主机、容器、网络、卷以及其他Docker资源。
```yaml
services:
  dockge:
    ...
    ports:
      - 9000:9000
    volumes:
      - /var/run/docker.sock:/var/run/docker.sock # docker服务目录
      - ./data:/data # 持久化文件存储目录
```

## Dockge
网址：[https://dockge.kuma.pet](https://dockge.kuma.pet)
```yaml
services:
  dockge:
    image: 'docker.olinl.com.cn/louislam/dockge:1.5.0'
    ...
    volumes:
      - /var/run/docker.sock:/var/run/docker.sock # docker服务目录
      - ./data:/app/data # 数据目录
      - /app/stacks:/app/stacks # docker-compose存储目录
    environment:
      - DOCKGE_STACKS_DIR=/app/stacks
```
...
services:
  vaultwarden:
    ...
    restart: always
    environment:
      DOMAIN: "https://passwd.jianglin.cc" # 域名
    volumes:
      - ./data/:/data/ # 持久化存储目录
    #ports: # 映射端口：实际使用时需要反向代理到域名
    #  - 80:80
```
## Lucky
...
services:
  lucky:
    ...
    ports:
      - 8401:16601 # 管理页面端口
      - 8443:8443 # 自定义https端口，默认443
      - 8440:8440 # 自定义http端口，默认80
    volumes:
      - ./data:/goodluck # 持久化目录端口
```
### http跳转https
```bash
#1. 新创建一条Web服务规则，注意这条新的Web服务规则不要启用TLS
#2. 在默认子规则中选择Web服务类型为 重定向
#3. 默认子规则

# https规则监听的是443端口
https://{host}{path}{args}
# https规则监听的是非443端口
https://{host}:{port}{path}{args}
```

## Alist

Alist 是一个搜索文件名、目录和内容的工具，具有快速、高效的搜索能力。这个镜像可以帮助你在你的 Docker 容器中快速地检索和管理文件名、目录和内容

> PS：前段时间Alist被收购，接着开源代码仓库爆出隐私问题，所以使用稳定版本：v3.40.0
> 这个版本已经上传至阿里云容器仓库保存，所以固定image标签
> 其他的安装教程还可以使用官方文档
...
services:
  alist:
    image: 'crpi-xntgazlqn787usm7.cn-shanghai.personal.cr.aliyuncs.com/olinl/alist:v3.40.0-ffmpeg'
    ...
    volumes:
      - './data:/opt/alist/data'
      # - './temp:/mnt/temp' # 临时存储目录，在存储管理把/mnt/temp添加为本机存储即可
    restart: always
    environment:
      - PUID=0
      - PGID=0
...
[https://docs.immich.app/features/hardware-transcoding](https://docs.immich.app/features/hardware-transcoding)
...
immich-machine-learning的标签 
https://docs.immich.app/features/ml-hardware-acceleration

对于硬件加速，添加一个 armnn(MaliGPU)，cuda(Nvidia)，rom(AMD)，openvino(Intel)，rknn(瑞芯微) 
...
[immich智能相册更换支持中文搜索的CLIP大模型教程](https://www.bilibili.com/opus/921308194821636097)




immich 反向地理编码汉化-将地图显示为国家、省份、城市
https://www.bilibili.com/opus/1039336133782143030
...
services:
  immich-server:
    ...
    image: crpi-xntgazlqn787usm7.cn-shanghai.personal.cr.aliyuncs.com/olinl/immich-server:release
    devices:
      - /dev/dri:/dev/dri
    volumes:
      - ./library:/data
      - ./external_libraries:/mnt/external_libraries:ro
      - /etc/localtime:/etc/localtime:ro
      
      
      - ./geodata-cn/geodata:/build/geodata
      - ./geodata-cn/i18n-iso-countries:/usr/src/app/node_modules/i18n-iso-countries
    ports:
      - '2283:2283'
    depends_on:
      - redis
  ...
  immich-machine-learning:
    ...
    image: crpi-xntgazlqn787usm7.cn-shanghai.personal.cr.aliyuncs.com/olinl/immich-machine-learning:release-openvino
    device_cgroup_rules:
      - 'c 189:* rmw'
    devices:
      - /dev/dri:/dev/dri
  ...
  redis:
    ...
    image: crpi-xntgazlqn787usm7.cn-shanghai.personal.cr.aliyuncs.com/olinl/valkey:8-bookworm
    healthcheck:
  ...
  database:
    ...
    image: crpi-xntgazlqn787usm7.cn-shanghai.personal.cr.aliyuncs.com/olinl/postgres:14-vectorchord0.4.3-pgvectors0.2.0
    environment:
...
## EmbyServer
Emby是一个收费的项目，这里使用的第三方的开心版。
[amilys/embyserver](https://hub.docker.com/r/amilys/embyserver)

使用的镜像：docker pull amilys/embyserver:latest
